...
idbroker_role: str = "arn:aws:iam::214178861886:role/test-role"
idbroker_instance_profile: str = "arn:aws:iam::214178861886:instance-profile/test-role"
datalake_admin_arn = "arn:aws:iam::214178861886:role/datalake_admin_role"
ranger_audit_arn = "arn:aws:iam::214178861886:role/datalake_admin_role"

config: Dict[str, Any] = {
    "env": {
        "aws": {
            "instance_profile": {"name": {"idbroker": f"{idbroker_instance_profile}"}},
            "role": {
                "name": {
                    "datalake_admin": f"{datalake_admin_arn}",
                    "ranger_audit": f"{ranger_audit_arn}",
                }
            },
        }
    },
...
def test_idbroker_instance_profile_with_role() -> None:
...
def test_idbroker_instance_profile_with_no_role() -> None:
    """Verify validation fails without a role within the idbroker instance profile."""
    iam_client: IAMClient = boto3.client("iam")
    stubber = Stubber(iam_client)
    add_get_profile_response(
        stubber=stubber,
...
def test_idbroker_role_with_no_trust_policy() -> None:
    """Verify validation fails without a trust policy in the idbroker role."""
...
def test_idbroker_role_with_trust_policy() -> None:
    """Verify validation passes with a trust policy in the idbroker role."""
...
def test_idbroker_role_with_no_assumerole_policy() -> None:
    """Verify validation fails without an assumerole policy in the idbroker role."""
    ...
    add_get_role_response(stubber, datalake_admin_arn, False)

    add_get_role_response(stubber, ranger_audit_arn, False)

    add_simulate_policy_response(
        stubber=stubber,
        role_arn=idbroker_instance_profile,
        resource_arns=[datalake_admin_arn, ranger_audit_arn],
...
def test_idbroker_role_with_assumerole_policy() -> None:
    """Verify validation passes with an assumerole policy in the idbroker role."""
...
def test_idbroker_role_with_valid_s3_policies(
...
def test_idbroker_role_with_invalid_s3_policies(
    logs_needed_actions: List[str],  # noqa: F811
) -> None:
    """Verify s3 policies validation passes with passing policy simulations."""
    iam_client: IAMClient = boto3.client("iam")
...
def test_idbroker_role_with_valid_s3_bucket_policies(
...
def test_idbroker_role_with_invalid_s3_bucket_policies(
    log_bucket_needed_actions: List[str],  # noqa: F811
) -> None:
    """Verify s3 policies validation passes with passing policy simulations."""
    iam_client: IAMClient = boto3.client("iam")
